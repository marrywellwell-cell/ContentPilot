...
**중요: 정확히 5장의 슬라이드를 생성하세요. 각 슬라이드는 제목 + 상세 내용을 포함해야 합니다.**

${brandContext ? "브랜드의 USP, 고객 페르소나, Pain Point, 솔루션을 반영하여 콘텐츠를 작성해주세요.\n\n" : ""}

## 슬라이드 구성 규칙
각 슬라이드 텍스트는 다음 형식으로 작성하세요:
[슬라이드 제목]
[빈 줄]
[상세 내용 2~3문장 또는 불릿 포인트 3개]

- 슬라이드 1 (커버): 주제를 임팩트 있게 표현 + 독자 관심을 끄는 문구
- 슬라이드 2~4: 핵심 정보 포인트 각각 — 제목 + 실용적이고 구체적인 설명
- 슬라이드 5 (마무리): 핵심 요약 + 팔로우/저장/댓글 유도 CTA
...
다음 JSON 형식으로 응답해주세요:
{
  ...
  "caption": "이모지가 포함된 300자 이상 전체 캡션 텍스트",
