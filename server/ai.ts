...
**중요: 정확히 5장의 슬라이드를 생성하세요.**

${brandContext ? "브랜드의 USP, 고객 페르소나, Pain Point, 솔루션을 반영하여 콘텐츠를 작성해주세요.\n\n" : ""}

## 슬라이드 텍스트 형식 (이미지 위에 올라가므로 가독성이 핵심)
각 슬라이드는 아래 형식으로 작성하세요. 문장이 아닌 키워드/축약어 위주로:
...
- 슬라이드 1 (커버): 이모지 + 주제 제목만 (예: "🌿 건강한 아침")
- 슬라이드 2~4: 이모지 제목 + 불릿 3개 (각 불릿은 5~8자 핵심 키워드)
- 슬라이드 5 (마무리): 이모지 + 요약 제목 + 저장/팔로우 유도 한 줄

## 캡션 규칙 (이미지 아래 텍스트 — 여기에 상세 내용 작성)
- 300자 이상 충분한 길이
- 이모지 적극 활용
- 훅(첫 줄) → 슬라이드별 상세 설명 → CTA 구조
- 실용적이고 유익한 정보 포함
...
다음 JSON 형식으로 응답해주세요:
{
  ...
  "caption": "이모지가 포함된 300자 이상 상세 캡션 텍스트",
  "hashtags": ["해시태그1", "해시태그2", ...]
        ...
        if (result.slides && Array.isArray(result.slides)) {
          result.slides = result.slides.slice(0, 5);
        }
...
"${keyword}" 주제에 대해 네이버 인기 블로그 스타일로 **충분히 길고 상세한** 글을 작성해줘.

${brandContext ? "브랜드의 USP, 고객 페르소나, Pain Point, 솔루션을 반영하여 콘텐츠를 작성해주세요.\n\n" : ""}
<작성 규칙>
- 제목과 첫 문단에 메인 키워드 "${keyword}"를 자연스럽게 넣기
- 첫 문장은 강력한 Hook으로 시작 (질문, 충격적 사실, 공감 유발)
- 소제목(H2/H3)을 활용해 **5~7개**의 본론 구성 (각 섹션당 200자 이상)
- 각 섹션에 구체적인 팁, 수치, 사례, 방법론을 포함
- 리스트(<ul><li>), 굵은글씨(<strong>), 표(<table>) 등을 활용해 가독성 강화
- 팩트 기반 정보 + 실제 경험 결합
- 독자에게 말하듯 친근하고 신뢰감 있는 톤 (반말 금지, 존댓말 사용)
- 전체 분량: 최소 1500자 이상 (충분히 상세하게)
- 결론에서는 핵심 요약 + 실천 가능한 행동 지침 3가지 + CTA 포함

<출력 형식>
다음 JSON 형식으로 응답해주세요:
{
  "title": "메인 제목 (SEO 최적화, 메인 키워드 포함, 클릭을 유도하는 매력적인 제목)",
  "metaDescription": "메타 설명 (120-150자, 검색 결과에 노출될 설명, 키워드 포함)",
  "content": "블로그 전체 내용 (순수 텍스트 버전, 1500자 이상)",
  "html": "블로그 전체 내용 (HTML 포맷팅 포함, H2/H3 소제목, 리스트, 굵은글씨, 표, 충분히 상세하게)",
