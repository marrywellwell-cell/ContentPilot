...
// Middleware that accepts both session auth AND Bearer API key
export const isAuthenticatedOrApiKey: RequestHandler = async (req: any, res, next) => {
  ...
  if (authHeader?.startsWith("Bearer ")) {
    ...
    if (token.startsWith("cf_")) {
      try {
        ...
        if (apiKey) {
          // Attach a synthetic user object so routes can get userId
          req.user = { claims: { sub: apiKey.userId } };
  ...
  const refreshToken = user.refresh_token;
  if (!refreshToken) { res.status(401).json({ message: "Unauthorized" }); return; }
  try {
    const config = await getOidcConfig();
    const tokenResponse = await client.refreshTokenGrant(config, refreshToken);
    updateUserSession(user, tokenResponse);
